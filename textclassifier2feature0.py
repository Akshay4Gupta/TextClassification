...
def wordsExtraction(data, case):
    ...
    for j in data.itertuples():
        ...
        for i in range(len(words)-1):
            newwords.append(' '.join([words[i], words[i+1]]))
        for i in range(len(words)-2):
            newwords.append(' '.join([words[i], words[i+1], words[i+2]]))
        dataBuilt[j[2]] = (j[1], newwords)
    return dataBuilt
...
def main():
    ...
    picfile = open("abackup", 'ab')
    pickle.dump((philyEquals, dicto, keysInDict, phi, m, wcyEquals, testingDataPreprocessed), picfile)
    picfile.close()
